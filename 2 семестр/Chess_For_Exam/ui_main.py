...
from ui_dialog import PlaceFigureDialog
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.place_user_figures_button = QPushButton(self.centralwidget)
        ...
        self.user_desk_size_input = QLineEdit(self.layoutWidget)
        self.user_desk_size_input.setObjectName(u"user_desk_size_input")

        self.user_required_figures_input = QLineEdit(self.layoutWidget)
    ...
    def place_user_figures(self):
        ...
        if self.user_desk_size_input.text() == "" or self.user_required_figures_input.text() == "":
            ...
            input_file.write(f"{self.N} {self.L}")
            input_file.close()

            dialog = PlaceFigureDialog()  # Передаем размер доски
            if dialog.exec() == QDialog.Accepted:
                positions = dialog.get_selected_positions()
                print("Выбранные позиции:", positions)
                # Дописываем выбранные позиции в файл
    ...
    def start(self):
        ...
        if self.user_desk_size_input.text() == "" or self.user_required_figures_input.text() == "":
            ...
            input_file.close()
            board = Board(self.N)
            self.existing_positions = [tuple(map(int, line.strip().split(" "))) for line in coords.readlines()]
            game = Game(board, self.L, self.existing_positions)
            game.start()
            output_coords = [x.strip(" ") for x in open("output.txt", "r").readline().replace(") ", ")*").split("*")]
            print(output_coords)
            cell_size = 399 // self.N
            for x in range(self.N):
                for y in range(self.N):
                    rect = QGraphicsRectItem(x * cell_size, y * cell_size, cell_size, cell_size)
                    if f"({x}, {y})" in output_coords:
                        rect.setBrush(QColor(0, 0, 255))
                    elif (x, y) in self.existing_positions:
