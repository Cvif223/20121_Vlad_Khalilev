...
class Game:
    ...
    def start(self, ui_solution = False):
        if self._e != []:
            for x, y in self._e:
            ...
            if ui_solution:
                self.place_figures_for_ui(self._L)
            else:
                self.place_figures(self._L)


    def place_figures_for_ui(self, l, current_solution=[], positions_now={0: (0, 0)}):
        print("place_figures_for_ui")
        if l == 0:
            print(self.first_solution)
            print("l = 0")
            if self.first_solution:
                data["board"] = copy.deepcopy(self._board)
                return True
        else:
            for x in range(positions_now[self._L - l][0], self._N):
                ...
                for y in range(start_y, self._N):
                    if self._board[x, y].is_free and not self._board[x, y].is_under_attack:
                        ...
                        self._attacked_figures(x, y)
                        if self.place_figures_for_ui(l - 1, current_solution, positions_now):
                            return True
